...
export default function ForecastCardDetails({
  forecastDetails,
  forecastDetailsRef,
}) {
  ...
  if (!forecastDetails) {
    return (
      <div
        ...
        className="flex justify-center flex-wrap m-1 p-1 w-auto h-auto bg-sky-400 rounded-sm text-stone-50 animate-pulse"
      >
        <p>Select a forecast to see more details...</p>
      </div>
    );
  }
  ...
  return (
    <div
      ref={forecastDetailsRef}
      className="flex justify-center flex-wrap m-1 p-1 w-auto h-auto bg-sky-400 rounded-sm text-stone-50 animate-fade-in-scale"
    >
      <ForecastDetailsItem
        name={"Condition"}
